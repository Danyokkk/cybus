...
// --- VERSION CHECK LOG ---
console.log("--- SERVER VERSION: V6 (SNI RESTORATION + WAF BYPASS) ---");
...
// --- THE FIX: Precision SSL Agent (V6) ---
// Restoring SNI while keeping rejectUnauthorized false for max resilience.
const httpsAgent = new https.Agent({
  rejectUnauthorized: false,
  servername: 'opendata.cyprusbus.transport.services', // RESTORE SNI (Fixes SSL 112)
  minVersion: 'TLSv1.2',                            // Base requirement for most WAFs
...
const axiosInstance = axios.create({
  ...
  headers: {
    ...
    'Referer': 'https://opendata.cyprusbus.transport.services/explore/dataset/emel-lemesos-gtfs-realtime/information/',
    'Origin': 'https://opendata.cyprusbus.transport.services',
...
async function processFeed(url, regionPrefix, tempPositions, tempUpdates) {
  try {
    const response = await axiosInstance.get(url, {
      ...
      timeout: 25000 // Increased timeout for slow government servers
    });
...
async function loadData() {
  ...
  stops = []; routes = []; trips = []; stopTimetable = {}; routeStops = {}; shapes = {}; routeShapes = {};
  ...
  for (const dir of dataDirs) {
    ...
    await processCSV(path.join(dir, stopsFile), (row) => {
      ...
      if (id && lat && lon && !stopsSet.has(id)) {
        stopsSet.add(id);
        stops.push({ stop_id: regionPrefix + id, name: name, lat: parseFloat(lat), lon: parseFloat(lon) });
      }
    });

    await processCSV(path.join(dir, 'routes.txt'), (row) => {
      routes.push({ route_id: regionPrefix + row.route_id, short_name: row.route_short_name, long_name: row.route_long_name, color: row.route_color, text_color: row.route_text_color });
    });
    ...
    await processCSV(path.join(dir, 'trips.txt'), (row) => {
      ...
      if (activeServices.has(fullServiceId)) {
        ...
        trips.push({ trip_id: fullTripId, route_id: fullRouteId, service_id: fullServiceId, trip_headsign: row.trip_headsign, shape_id: fullShapeId });
        activeTripsSet.add(fullTripId);
        regionTripToRoute.set(fullTripId, fullRouteId);
        if (fullShapeId) { if (!routeShapes[fullRouteId]) routeShapes[fullRouteId] = new Set(); routeShapes[fullRouteId].add(fullShapeId); }
      }
    });
    ...
    await processCSV(path.join(dir, 'stop_times.txt'), (row) => {
      ...
      if (activeTripsSet.has(fullTripId)) {
        ...
        if (routeId) { if (!routeStops[routeId]) routeStops[routeId] = new Set(); routeStops[routeId].add(fullStopId); }
      }
    });
    ...
    await processCSV(path.join(dir, 'shapes.txt'), (row) => {
      ...
      tempShapes[shapeId].push({ lat: parseFloat(row.shape_pt_lat), lon: parseFloat(row.shape_pt_lon), seq: parseInt(row.shape_pt_sequence) });
    });
    Object.keys(tempShapes).forEach(sid => { shapes[sid] = tempShapes[sid].sort((a, b) => a.seq - b.seq).map(pt => [pt.lat, pt.lon]); });

    regionTripToRoute.clear(); activeTripsSet.clear(); if (global.gc) global.gc();
...
app.get('/api/stops/:stopId/timetable', (req, res) => {
  ...
  for (const item of rawArrivals) {
    ...
    let arrivalTime = item.a; let isRealtime = false; let delay = 0;
    if (tripUpdates[item.t] && tripUpdates[item.t][stopId]) {
      const update = tripUpdates[item.t][stopId];
      if (update.arrival_time) { const dateObj = new Date(update.arrival_time * 1000); arrivalTime = dateObj.toLocaleTimeString('en-GB', { hour12: false }); isRealtime = true; delay = update.delay; }
    }
    results.push({ route_short_name: route ? route.short_name : '?', trip_headsign: trip.trip_headsign, route_id: route ? route.route_id : '?', arrival_time: arrivalTime, is_realtime: isRealtime, delay: delay });
  }
